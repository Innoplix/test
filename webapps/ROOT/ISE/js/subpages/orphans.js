 var orphans = {
    ...
    init: function() {		
		...
		$(".details #markAsCreateBtn").live("click", function(e){
				...
				var jsonString='{"requesttype":"updateObjectModel","collection":"defects","columnname":"_id","columnvalue":"'+orphans.defectIDValue+'","object":'+JSON.stringify(jsonStrObj)+',"projectName":"' + localStorage.getItem('projectName') + '","fromCache":"false"}';
		...
		$("#orphanTestcasePopup #testCaseSubmit").live("click", function(e){
...
saveChanges:function(){
	...
	addOrphanBucket:function(eachFeature,isPopup) {
		var bucket = {
			...
			prtltPopupTCP : $("<a>", {"data-toggle":"modal", onclick:"javascript:orphans.renderPopupP('"+eachFeature.name+"','TC');", href:"#largeorphP"}),
			prtltPopupIDP : $("<a>", {"data-toggle":"modal", onclick:"javascript:orphans.renderPopupP('"+eachFeature.name+"','ID');", href:"#largeorphP"}),
			prtltPopupIDOP : $("<a>", {"data-toggle":"modal", onclick:"javascript:orphans.renderPopupP('"+eachFeature.name+"','IDO');", href:"#largeorphP"}),
			prtltPopupEDP : $("<a>", {"data-toggle":"modal", onclick:"javascript:orphans.renderPopupP('"+eachFeature.name+"','ED');", href:"#largeorphP"}),
			prtltPopupEDOP : $("<a>", {"data-toggle":"modal", onclick:"javascript:orphans.renderPopupP('"+eachFeature.name+"','EDO');", href:"#largeorphP"}),
		...
		else
		{
         ...
         tableRow+="<td><a data-toggle='modal' onclick=orphans.renderPopupP('"+escape(eachFeature.name)+"','TC'); href=#largeorphP>"+eachFeature.tcCount+"</a></td>";
         else
         tableRow+="<td style=color:#7dc1eb >"+eachFeature.tcCount+"</td>";

         if(eachFeature.intDefCount!=0)
         tableRow+="<td><a data-toggle='modal' onclick=orphans.renderPopupP('"+escape(eachFeature.name)+"','ID'); href=#largeorphP>"+eachFeature.intDefCount+"</a></td>";
         else
         tableRow+="<td style=color:#7dc1eb >"+eachFeature.intDefCount+"</td>";

         if(eachFeature.intOrphDefCount!=0)
         tableRow+="<td><a data-toggle='modal' onclick=orphans.renderPopupP('"+escape(eachFeature.name)+"','IDO'); href=#largeorphP>"+eachFeature.intOrphDefCount+"</a></td>";
         else
         tableRow+="<td style=color:#7dc1eb >"+eachFeature.intOrphDefCount+"</td>";

         if(eachFeature.extDefCount!=0)
         tableRow+="<td><a data-toggle='modal' onclick=orphans.renderPopupP('"+escape(eachFeature.name)+"','ED'); href=#largeorphP>"+eachFeature.extDefCount+"</a></td>";
         else
         tableRow+="<td style=color:#7dc1eb >"+eachFeature.extDefCount+"</td>";

         if(eachFeature.extOrphDefCount!=0)
         tableRow+="<td><a data-toggle='modal' onclick=orphans.renderPopupP('"+escape(eachFeature.name)+"','EDO'); href=#largeorphP>"+eachFeature.extOrphDefCount+"</a></td>";
         else
	...
	renderPopup:function(feature,srcEvent,e,event) {
		...
		var projectName = localStorage.getItem('projectName');
		if (feature == "Others")
			var filter = "_missing_:primary_feature OR primary_feature:\"\"";
		else
	...
	_receivedOrphanResultsPopup:function(data){
		if(null !=data && undefined != data && data.length>0){
			...
			if (orphans.m_feature == "Others")
				...
				var filter = 'primary_feature_parent:"'+orphans.m_feature+'" primary_feature:"'+orphans.m_feature+'"';
...
.replaceAll("&lt;em","<em").replaceAll("&lt;/em&gt;","</em>").replaceAll("'iseH'&gt;","'iseH'>").replaceAll("'iseH'&lt;","'iseH'>"));
	...
	renderPopupP:function(feature,srcEvent) {
		orphans.m_feature = unescape(feature);
		orphans.m_srcEvent=srcEvent;
		$('#searchResultsTable_orphansDP').hide();
		$('#searchResultsTable_orphansTP').hide();
			
		var filter = 'primary_feature_parent:"'+unescape(orphans.m_feature)+'" primary_feature:"'+unescape(orphans.m_feature)+'"';
			if(orphans.m_srcEvent == "ID")
				filter = filter +'  AND internaldefect: \"1\" AND _exists_:testcases';
			if(orphans.m_srcEvent == "IDO")
				filter = filter +'  AND internaldefect: \"1\" AND _missing_:testcases';
	...
	markAsCreateBtn:function(data){
	  ...
	  confirmTestCase:function(testcaseIdValue,defectIdValue){
			...
			var jsonString='{"requesttype":"updateJsonArrayFieldByIds","collection":"defects","columnname":"_id","columnvalue":"'+defectIdValue+'","testcaseName":"test_case_id","testcaseValue":"'+testcaseIdValue+'","testcaseStatusName":"status","testcaseStatusValue":"Confirmed","jsonArrayField":"testcases","projectName":"' + localStorage.getItem('projectName') + '","fromCache":"false"}';
	  ...
	  updateTestCase:function(testcaseIdValue,defectIdValue){
		  	...
		  	var jsonString='{"requesttype":"updateJsonArrayFieldByIds","collection":"defects","columnname":"_id","columnvalue":"'+defectIdValue+'","testcaseName":"test_case_id","testcaseValue":"'+testcaseIdValue+'","testcaseStatusName":"action","testcaseStatusValue":"mark_for_update","jsonArrayField":"testcases","projectName":"' + localStorage.getItem('projectName') + '","fromCache":"false"}';
